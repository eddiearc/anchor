...
export async function runCli(args: string[], options: CliOptions = {}): Promise<CliResult> {
  ...
  if (command === "plan") {
    return json(await runPlan(rest, storePath, tasksDir));
  }
  ...
  if (command === "generate") {
    return json(await runGenerate(rest, storePath, tasksDir, worktreesDir));
  }

  if (command === "evaluate") {
    return json(await runEvaluate(rest, storePath, tasksDir, worktreesDir));
  }

  if (command === "run-retry") {
    return json(await runRetry(rest, storePath, tasksDir, worktreesDir));
  }
...
async function runPlan(args: string[], storePath: string, tasksDir: string) {
  ...
  const contract = await writeContractArtifact(tasksDir, taskId, createTemplateContract(taskStr, taskId));
  const produced = await store.appendEvent(
    taskId,
    {
      type: "CONTRACT_PRODUCED",
      mode: "standard",
      reasoning: "Deterministic contract generated by CLI.",
      affected_scope: ["src/**", "tests/**", "README.md", "package.json", "tsconfig*.json"],
...
async function runGenerate(args: string[], storePath: string, tasksDir: string, worktreesDir: string) {
  const taskId = args[0];
  if (!taskId) {
    return { ok: false, error: "task_id_required", storePath, tasksDir, worktreesDir };
  }
...
async function runEvaluate(args: string[], storePath: string, tasksDir: string, worktreesDir: string) {
  const taskId = args[0];
  if (!taskId) {
    return { ok: false, error: "task_id_required", storePath, tasksDir, worktreesDir };
  }
...
async function runRetry(args: string[], storePath: string, tasksDir: string, worktreesDir: string) {
  const taskId = args[0];
  if (!taskId) {
    return { ok: false, error: "task_id_required", storePath, tasksDir, worktreesDir };
  }
