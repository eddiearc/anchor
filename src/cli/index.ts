...
type GitRootResult = { ok: true; root: string } | { ok: false };
...
export async function runCli(args: string[], options: CliOptions = {}): Promise<CliResult> {
  ...
  const git = options.repoPath ? { ok: true as const, root: options.repoPath } : await gitRoot();
  ...
  const config: AnchorConfig = options.config ?? await loadAnchorConfig();
...
async function runRun(args: string[], storePath: string, tasksDir: string, config: AnchorConfig, git: GitRootResult) {
  const result: any = await runPlan(args, storePath, tasksDir, config, git);
  if (!result.ok) {
...
async function runWait(args: string[], storePath: string, tasksDir: string, worktreesDir: string, config: AnchorConfig, git: GitRootResult) {
  ...
  if (!taskId) {
    const run: any = await runRun(cleanArgs, storePath, tasksDir, config, git);
    steps.push({ command: "run", state: run.state ?? null, ok: run.ok });
    if (!run.ok) return { ...run, command: "run-wait", steps };
    taskId = run.taskId;
  }
  ...
  for (let stepIndex = steps.length; stepIndex < maxStepsResult.maxSteps; stepIndex++) {
    ...
    if (state === null) {
      ...
      if (!planned.ok) return { command: "run-wait", ...planned, steps };
      continue;
    }
    ...
    if (state === "REVIEW") {
      ...
      if (!reviewed.ok) return { command: "run-wait", ...reviewed, steps };
      continue;
    }
    ...
    if (state === "BUILD") {
      ...
      if (!workspace.ok) return { command: "run-wait", ...workspace, steps };
      ...
      if (!generated.ok) return { command: "run-wait", ...generated, steps };
      continue;
    }
    ...
    if (state === "CHECK") {
      ...
      if (!evaluated.ok) return { command: "run-wait", ...evaluated, steps };
      continue;
    }
...
async function runNext(taskId: string | undefined, storePath: string, tasksDir: string, worktreesDir: string, config: AnchorConfig) {
  if (!taskId) {
...
async function runPlan(args: string[], storePath: string, tasksDir: string, config: AnchorConfig, git: GitRootResult) {
  if (!git.ok) {
    return {
      ok: false,
      error: "not_git_repo",
...
async function producePlanForTask(
  taskId: string,
  ...
  config: AnchorConfig,
  git: GitRootResult,
  adapter = readOption(args, "--provider") ?? readOption(args, "--adapter") ?? defaultProvider(config, "planner")
) {
  if (!git.ok) {
    return {
      ok: false,
...
async function runGenerate(args: string[], storePath: string, tasksDir: string, worktreesDir: string, _config: AnchorConfig) {
  ...
  const eventResult = await store.appendEvent(
    taskId,
    { type: "CODE_PRODUCED", report_path: result.reportPath, files_changed: result.filesChanged, attempt, provider: result.report.provider, step_id: snapshot.context.currentStepId },
    "generator"
  );
  if (!eventResult.ok) {
...
async function runRetry(args: string[], storePath: string, tasksDir: string, worktreesDir: string, _config: AnchorConfig) {
  ...
  while (state === "BUILD" || state === "CHECK") {
    ...
    if (state === "BUILD") {
      ...
      const eventResult = await store.appendEvent(
        taskId,
        { type: "CODE_PRODUCED", report_path: result.reportPath, files_changed: result.filesChanged, attempt, provider: result.report.provider, step_id: loopContext.currentStepId },
        "generator"
      );
      if (!eventResult.ok) {
...
function stripOption(args: string[], option: string) {
  const index = args.indexOf(option);
...
function readMaxSteps(args: string[]): { ok: true; maxSteps: number } | { ok: false; code: "INVALID_MAX_STEPS"; message: string; detail: string } {
...
function resolveDefaultPath(explicitPath: string | undefined, defaultPath: string, git: GitRootResult): string {
  if (explicitPath) return explicitPath;
  if (git.ok) return path.resolve(git.root, defaultPath);
  return defaultPath;
}

function defaultProvider(config: AnchorConfig, role: "planner" | "reviewer" | "generator" | "evaluator"): string {
  const roleKey = `${role}_provider` as keyof AnchorConfig;
  const roleProvider = config[roleKey];
  if (typeof roleProvider === "string" && roleProvider.trim()) return roleProvider.trim();
  if (typeof config.provider === "string" && config.provider.trim()) return config.provider.trim();
  if (typeof config.agent === "string" && config.agent.trim()) return config.agent.trim();
  return "codex";
}

async function gitRoot(): Promise<GitRootResult> {
  try {
...
function nextCommandsForState(taskId: string, state: State | null | undefined, hasContract: boolean, config: AnchorConfig): string[] {
...
function nextActionsForState(taskId: string, state: State | null | undefined, hasContract: boolean, config: AnchorConfig) {
  if (state === "HUMAN") {
    return hasContract
      ? [
          {
  ...
  if (state === "CHECK") {
    ...
    if (provider === "fixture") command.push("--verdict", "pass");
    return [
      {
        action: "evaluate",
        command,
