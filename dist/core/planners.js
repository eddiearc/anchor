...
import { buildCodexArgv, codexCommand, runAgent, defaultCommandRunner, summarizeOutput } from "./agent-runner.js";
import { composePrompt } from "./config.js";
// ── dispatch ──
export async function runPlanner(input, runner = defaultCommandRunner) {
...
// ── fixture ──
export async function runFixturePlanner(input) {
...
// ── codex ──
async function runCodexPlanner(input, runner) {
    ...
    const result = await runAgent(cmd, argv, input.repoPath, runner);
...
function buildPlannerPrompt(input, contractPath) {
    ...
    const base = [
        ...
        "      criteria:",
        "        - id: '1.1'",
        "          description: <same testable criterion>",
        "          passes: false",
        "          evidence_required:",
        "            - command_output|test_output|file_path",
        "  completion_gate:",
        "    type: all",
        "    conditions:",
        "      - <condition>",
...
// ── parse ──
function parsePlannerOutput(yaml) {
...
function buildTemplateContract(taskDescription, taskId, mode = "standard") {
    return [
        `mode: ${mode}`,
        ...
        `            - test_output`,
        `        - id: "1.2"`,
        `          description: "TypeScript compilation succeeds with --strict"`,
        `          passes: false`,
        `          evidence_required:`,
        `            - command_output`,
        `  completion_gate:`,
