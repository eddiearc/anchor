...
export async function readWorkspaceStatus(artifactsDir, taskId) {
...
export async function readWorkspaceMetadata(artifactsDir, taskId) {
    try {
...
export async function writeWorkspaceMetadata(artifactsDir, taskId, metadata) {
    const filePath = workspaceMetadataPath(artifactsDir, taskId);
    await mkdir(path.dirname(filePath), { recursive: true });
    await writeFile(filePath, `${JSON.stringify(metadata, null, 2)}\n`);
}
export function workspaceMetadataPath(artifactsDir, taskId) {
...
export async function getWorkspaceGitStatus(worktreePath) {
...
async function git(args) {
    try {
        ...
        const detail = error instanceof Error && "stderr" in error && typeof error.stderr === "string" ? error.stderr.trim() : String(error);
        return {
            ok: false,
